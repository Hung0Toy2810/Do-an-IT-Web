// src/components/BestSellerProducts.tsx
...
const BestSellerProducts: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch('http://localhost:5067/api/bestseller/top-30');
    ...
    return (
      <section className="py-16 bg-violet-50 from-violet-50 via-purple-50 to-pink-50">
        <div className="px-4 mx-auto text-center max-w-7xl sm:px-6 lg:px-8">
          ...
          <p className="mt-4 text-violet-700">Đang tải sản phẩm bán chạy...</p>
        </div>
      </section>
    );
  }
